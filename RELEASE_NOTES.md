...
- **신규 — Teaching Control(직접 교시) 탭 (Windows 인스톨러 2026-07-22 재빌드)**
  - 엔드이펙터에 세 번째 탭 추가: **자유 이동**(전 모터 토크 해제)으로 손으로 팔을 끌어
    포즈를 잡고 시퀀스로 저장 → **Motion Record** 로 순차 실행·모션 저장(속도/힘 조절)
    → **Motion Replay** 로 재생(Real 송신 지원)
  - 토크 재활성 시 현재 자세 유지(점프 방지), 탭 이탈 시 토크 자동 복구 등 안전 가드 포함
  - ※ 현재 Windows 인스톨러에만 반영 — Linux .deb 는 다음 재빌드 시 포함 예정
...
- `.exe` SHA256: `94a508e467ef6d553c01e89c2da12847c0e0f65c1233504d808521454bf87bac`
